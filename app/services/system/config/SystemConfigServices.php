...
class SystemConfigServices extends BaseServices implements ServeConfigInterface
{
    ...
    {
        $formbuider = [];
        $from = $store_id > 0 ? config('admin.store_prefix') : config('admin.admin_prefix');
        switch ($type) {
            case 1:
                $data['value'] = json_decode($data['value'], true) ?: '';
...
//                            ->falseValue('关闭', 0)->trueValue('开启', 1)->info($data['member_price_status']['desc']),
    ...
    public function deliverFormBuild()
    {
        ...
        $data = $this->getConfigAllField([
            'store_free_postage', 'offline_postage',
//			'store_self_mention'
        ]);

        $build->rule([
            Build::card('发货设置')->components([
                Build::switch('whole_free_shipping', '全场包邮', (int)$data['store_free_postage']['value'] > 0 ? 1 : 0)
                    ->falseValue('关闭', 0)->trueValue('开启', 1)->control(1, [
...
//                Build::switch('store_self_mention', '是否开启到店自提', (int)$data['store_self_mention']['value'] > 0 ? 1 : 0)
//                    ->falseValue('关闭', 0)->trueValue('开启', 1)->info($data['store_self_mention']['desc'] ?? ''),
    ...
    public function cityDeliverFormBuild()
    {
        ...
        $data = $this->getConfigAllField([
            'city_delivery_status', 'self_delivery_status', 'dada_delivery_status', 'dada_app_key', 'dada_app_sercret', 'dada_source_id', 'uupt_appkey', 'uu_delivery_status', 'uupt_app_id', 'uupt_open_id','qipei_delivery_status'
        ]);
            ...
            Build::card('同城配送')->components([
                Build::switch('city_delivery_status', $data['city_delivery_status']['info'], (int)$data['city_delivery_status']['value'])
                    ->falseValue('关闭', 0)->trueValue('开启', 1)->control(1, [
                        ...
                        Build::input('qipei_delivery_status', $data['qipei_delivery_status']['info'], (int)$data['qipei_delivery_status']['value'])->info($data['qipei_delivery_status']['desc']),
...
//            ])
    ...
    {
        $data = $this->getConfigAllField([
            'routine_appId', 'routine_appsecret', 'routine_contact_type', 'routine_name', 'store_user_avatar'
        ]);
        return $data;
    }
    ...
    public function shopPayFormBuild()
    {
        ...
        $build->rule([
            Build::tabs()->option('微信支付', [
                ...
                Build::switch('is_cashier_yue_pay_verify', $data['is_cashier_yue_pay_verify']['info'], (int)$data['is_cashier_yue_pay_verify']['value'])->trueValue('是', 1)->falseValue('否', 0)->info($data['is_cashier_yue_pay_verify']['desc'])
            ])->option('线下支付', [
    ...
    public function shopTradeFormBuild()
    {
        ...
        $data = $this->getConfigAllField([
            ...
            'system_delivery_time', 'refund_name', 'refund_phone', 'refund_address', 'stor_reason','collate_not_operating_time',
        ...
        foreach (['order_cancel_time', 'order_activity_time',
                     'order_bargain_time', 'order_seckill_time', 'order_pink_time', 'rebate_points_orders_time', 'table_code_not_operating_time', 'collate_not_operating_time'] as $item) {
