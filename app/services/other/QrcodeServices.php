    ...
    public function getRoutineQrcodePath(int $id, int $uid, int $type, array $parame = [], bool $isSaveAttach = true)
    {
        ...
        switch ($type) {
            ...
            case 9: //桌码二维码
                ...
                $namePath = 'routine_table_code_' . $parame['store_id'] . '_' . $parame['table_number'] . '_' . $id . '.jpg';
                $data = 'qrcode_id=' . $id. '&store_id=' . $parame['store_id'];
                break;
            case 10: //门店二维码
                $page = 'pages/store_cate/store_cate';
                $namePath = 'routine_store_cate_id_' . $id . '.jpg';
                $data = 'id=' . $id;
                break;
            case 11: //线下扫门店二维码支付
                $page = 'pages/store_cate/store_cate';
                $namePath = 'routine_store_scan_code_id_' . $id .'_' .$parame['device_id']. '.jpg';
                $data = 'id=' . $id .'&device_id=' .$parame['device_id'];
                break;
        }
        if (!$page || !$namePath) {
            return false;
