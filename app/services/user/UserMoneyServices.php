...
class UserMoneyServices extends BaseServices
{
    ...
    protected $incomeData = [
        ...
        'offline_scan_refund' => [
            ...
            'mark' => '退款金额{%price%}元,[{%storeName%}]',
            'status' => 1,
            'pm' => 0
        ],
        'offline_scan_yue_pay' => [
            'title' => '线下扫码余额支付',
            'type' => 'offline_scan_yue_pay',
            'mark' => '余额支付{%price%}元,[{%storeName%}]门店扫码',
            'status' => 1,
            'pm' => 0
        ],
    ];
	...
	* @var string[]
     ...
     * @return array
     */
   public function getUserMoneyListAdmin(int $uid = 0, $where_time = [], string $field = '*',$paid=null,$phone=null,$excel=0)
    {
        
        if ($excel){
            [$page, $limit] = [1,99999999999];
        }else{
            [$page, $limit] = $this->getPageValue();
        }
        $where = [];
        ...
        if ($phone) {
            ...
            //$userInfo = User::withTrashed ()->where('phone','like','%'.$phone."%")->field('uid')->select();
        ...
        if ($where_time) {
            ...
            $where['add_time'] = explode('-',$where_time);
        }
        ...
        if ($paid) {
            switch ($paid) {
                case 'refund':
                    ...
                    break;
                case 'system_add':
                    $where['type'] = 'system_add';
                    break;
                case 'system_sub':
                    $where['type'] = 'system_sub';
                    break;
                case 'lvguxiaofei':
                    $where['type'] = 'pay_product';
                    $where['link_id'] = 0;
                    break;
                case 'otherxiaofei':
                    $where['type'] = 'pay_product';
        ...
        $where['not_type'] = 'recharge';
        $list = $this->dao->getList($where, $field, $page, $limit, ['user' => function ($query) use ($phone) {
            $query/*->where('phone','like',"%".$phone."%")*/ ->field('uid,nickname,phone');
        }]);
        $count = $this->dao->count($where);
        foreach ($list as &$item) {
            $value = array_filter($this->incomeData, function ($value) use ($item) {
    ...
    public function getRechargeSum(int $uid = 0, $time = [])
    {
        ...
        $where = ['uid' => $uid, 'pm' => 1, 'status' => 1, 'type' => ['recharge','system_add', 'user_recharge', 'brokerage_to_nowMoney', 'lottery_give_money', 'newcomer_give_money', 'level_give_money']];
    ...
    public function userMoneyList(int $uid, int $type, array $data = [])
    {
        ...
        switch ($type) {
            ...
            case 2:
                $where['pm'] = 1;
                 $where['type'] = [
                    'recharge','system_add', 'user_recharge', 
                    'brokerage_to_nowMoney', 
                    'lottery_give_money',
                    'newcomer_give_money', 'level_give_money'];//充值去掉退款
                break;
            case 0:
            default:
                break;
