...
class UserServices extends BaseServices
{
	...
	*/
     ...
     * @return array
     */
    public function index(array $where,$limit=0)
    {
        /** @var UserWechatuserServices $userWechatUser */
        $userWechatUser = app()->make(UserWechatuserServices::class);
        ...
        [$list, $count] = $userWechatUser->getWhereUserList($where, $fields,$limit);
        if ($list) {
            ...
            foreach ($list as  $i=>&$item) {
                if (!empty($where['uid']) && ! in_array($item['uid'],
                                $where['uid'])){
                    unset($list[$i]);
                    continue;
                }
...
//							throw new AdminException( '请填写正确的身份证号码');
	...
	*/
	public function saveExtendForm(int $uid, array $extend_info, array $update = [], bool $is_all = false)
	{
		...
		if ($update) {
		    if (isset($update['fuyou_pwd']) && $update['fuyou_pwd']){
		        $fuyouservices = app()->make(FuyouServices::class);
		        $rs = $fuyouservices->editFuyouUserInfo($userInfo['phone'],$update['fuyou_pwd']);
                if (! isset($rs['respCode']) ||  $rs['respCode'] != '000000') {
                    Log::error('富友改密错误');
                   return false;
                }
            }else {
    ...
    public function getHeaderList(int $uid, $userInfo = [])
    {
        ...
        return [
            ...
            [
                'title' => '总计订单',
                'value' =>  Db::name('user_money')->where('uid',$uid)->where('type','pay_product')->count()- Db::name('user_money')->where('uid',$uid)->where('type','pay_product_refund')->count(),//$orderServices->count($where),
                'key' => '笔',
            ],
            [
                'title' => '总消费金额',
                'value' => round(Db::name('user_money')->where('uid',$uid)->where('type','pay_product')->sum('number')- Db::name('user_money')->where('uid',$uid)->where('type','pay_product_refund')->sum('number'),2),//$orderServices->together($where, 'pay_price'),
                'key' => '元',
            ],
            [
                'title' => '积分',
            ...
            [
                'title' => '消费次数',
                'value' => Db::name('user_money')->where('uid',$uid)->where('type','pay_product')->count(),/*$orderServices->count(['uid' => $uid, 'pid' => [0, -1], 'paid' => 1, 'is_del' => 0, 'is_system_del' => 0]),*/
                'key' => '',
            ],
//            [
//                'title' => '本月订单',
...
//                $services = app()->make(UserSpreadServices::class);
    ...
    public function personalHome(array $user, $tokenData)
    {
        ...
        $user['orderStatusSum'] = round((float)($userMoneyServices->sum(['uid' => $uid, 'pm' => 0, 'status' => 1], 'number', true)
            -$userMoneyServices->sum(['uid' => $uid, 'pm' => 1, 'status' => 1,'type'=>'pay_product_refund'], 'number', true)),2);//消费 2023年7月21 消费加入退款求和
