...
class StoreCategoryServices extends BaseServices
{
     ...
     */
    public function createForm($id = 0, $type= '')
    {
		$info = [];
		if ($id) {
		...
		$url = $type ? config('admin.store_prefix') . '/widget.images/index' : config('admin.admin_prefix') . '/widget.images/index';
		
		$f[] = Form::input('name', '分类名称', $info['name'] ?? '')->maxlength(30)->required();
		$f[] = Form::number('sort', '排序', (int)($info['sort'] ?? 0))->min(0)->min(0);
		$f[] = Form::frameImage('pic', 'ICON图(180*180)', Url::buildUrl($url, array('fodder' => 'pic')), $info['pic'] ?? '')->icon('ios-add')->width('960px')->height('505px')->modal(['footer-hide' => true]);
