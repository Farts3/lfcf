    ...
    public function applyRefund(int $id, int $uid, $order = [], array $cart_ids = [], int $refundType = 0, float $refundPrice = 0.00, array $refundData = [], int $origin = 0, bool $isSync = true)
    {
        ...
        if ($cart_ids) {
            ...
            if ($total_num < $order['total_num']) {
                ...
                if (isset($order['change_price']) && $order['change_price']) {//有改价 且是拆分
					//订单原实际支付金额
...
//                $res2 = false !== $this->storeOrderServices->update(['id' => $order['id']], ['refund_status' => 1]);
    ...
    public function cancelOrderRefundCartInfo(int $id, int $oid, $orderRefundInfo = [])
    {
		...
		$orderInfo = $storeOrderServices->get($oid, ['id', 'type', 'pink_id', 'activity_id']);
		if (!$orderInfo) {
			throw new ValidateException('订单不存在');
		}
		//拼团订单处理拼团
		if ($orderInfo['type'] == 3 && $orderInfo['pink_id']) {
			/** @var StorePinkServices $pinkServices */
			$pinkServices = app()->make(StorePinkServices::class);
			$addTime = $pinkServices->value(['id' => $orderInfo['pink_id']], 'add_time');
			...
			$pinkServices->update(['id' => $orderInfo['pink_id']], ['status' => 1, 'stop_time' => $addTime + $product['effective_time'] * 3600]);
		}
                    ...
                    case PayServices::WEIXIN_PAY:
                        ...
                        if ($refundOrder['is_channel'] == 1) {
                            ...
                            if($pay_routine_open)
                            {
                                ...
                                }else{$refundData['routine_order_id'] = $refundOrder['order_id'];}
    ...
    public function yueRefund($order, array $refundData)
    {
        ...
        $fuyouservices->adjustFuyouByUid($order['uid'],bcadd((string)$usermoney, (string)$refundData['refund_price'], 2));
	...
	*/
    public function regressionIntegral($order)
    {
		...
		Log::error('回退积分结果'.'uid'.$order['uid'].'积分'.$integral.'结果:'.$res3);
        if (!($res1 && $res2 && $res3)) {
             Log::error('回退积分增加失败'.'uid'.$order['uid'].'积分'.$integral);
