...
class UserMoneyDao extends BaseDao
{
    ...
    public function getList(array $where, string $field = '*', int $page = 0, int $limit = 0, array $with = [])
    {
     ...
     //变动金额查询后台
    public function getList1(array $where, string $field = '*', int $page = 0, int $limit = 0, array $with = [],string $phone=null)
    {
        
        return $this->search($where)->hasWhere('user',[["phone","like","%".$phone."%"]])->field($field)/*->when($with, function ($query) use ($with) {
            $query->with($with);
        })*/->when($page && $limit, function ($query) use ($page, $limit) {
            $query->page($page, $limit);
        })->order('id desc')->select()->toArray();
       return $this->search($where)->when($phone,function ($q)use ($phone){
            return $q->hasWhere('user',[["phone","like","%".$phone."%"]]);
