...
class Common extends AuthController
{
    ...
    // if (!function_exists('queryUserInfo')) {
    
    //分页查询会员信息
    function queryUserInfo(){
        $fuyouurl="https://spht-test.fuioupay.com/api";
    // $fuyouurl="https://sp-ht.fuioupay.com/api/queryUserInfo.action";
    $mchntCd="0002900F0313432";
    $openId="oESfm1ZyxsWF-wGEiM-vw1MjR5mE";
    $salt="XyUbZQmfejRMQMUQ79SBCBORgWs9A38x";
    $shopId="22258";
        $url=$fuyouurl."/queryUserInfo.action";
        $sign=md5($mchntCd."|". $shopId."|" .$salt);
        $data=array(
            'mchntCd'=>$mchntCd,
            'shopId'=>$shopId,
            'key'=>$sign
            );
        $res=php_curl_https($url,$data);
        echo json_encode($res);
    }
// }

// if (!function_exists('php_curl_https')) {
    //https
    function php_curl_https($url,$data){
    	$ch = curl_init();  
    	$timeout = 300;   
    ...
    public function jnotice()
    {
        ...
        if ($inventory) {
            $value[] = [
                ...
                'type' => 'information',
                'url' => '/product/product_list?type=5',
            ];
        }
        if ($commentNum) {
            $value[] = [
        ...
        if ($reflectNum) {
            $value[] = [
                ...
                'type' => 'bulb',
                'url' => '/finance/user_extract/index?status=0',
            ];
        }
        if ($orderRefundNum) {
            $value[] = [
