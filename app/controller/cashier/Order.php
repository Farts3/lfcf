        ...
        try {
            ...
            if (isset($res['status']) && $res['status'] === 'SUCCESS') {
                ...
                $fuyouServices = \app()->make(FuyouServices::class);
                  $fuyouServices->adjustFuyouByUid($userInfo['uid'],bcsub((string)$userInfo['now_money'],$computeData['payPrice'],2));
                  //改富友积分
                $pointdata = $fuyouServices->queryPoint($userInfo['phone']);
                $fuyouServices->editPoint($userInfo['phone'], $pointdata['point'], bcadd((string)$userInfo['integral'],(string)$computeData['payPrice']));
