...
class StoreOrderController
{
    ...
    public function confirm(Request $request, ShippingTemplatesServices $services)
    {
        ...
        [$cartId, $new, $addressId, $shipping_type, $storeId, $couponId, $luckRecordId] = $request->postMore([
            'cartId',
            'new',
            ['addressId', 0],
            ['shipping_type', 1],
        ...
        return app('json')->successful($this->services->getOrderConfirmData($user, $cartId, !!$new, (int)$addressId, (int)$shipping_type, (int)$storeId, (int)$couponId, (int)$luckRecordId));
    }
    ...
    public function create(Request $request, StoreOrderCreateServices $createServices, $key)
    {
        ...
        [$addressId, $couponId, $payType, $useIntegral, $mark, $combinationId, $pinkId, $seckill_id, $collate_code_id, $bargainId, $newcomerId, $from, $shipping_type, $real_name, $phone, $storeId, $news, $invoice_id, $quitUrl, $discountId, $customForm, $discount_price] = $request->postMore([
            [['addressId', 'd'], 0],
            [['couponId', 'd'], 0],
            ['payType', ''],
            ['useIntegral', 0],
        ...
        if ($orderId) {
            ...
            // 打折金额，zz
            if($discount_price) {
                $orderInfo['pay_price'] = ($orderInfo['pay_price'] * 100 * $discount_price) / 10000;
                $orderInfo['pay_price'] = number_format($orderInfo['pay_price'], 2);    
            }
            ...
            switch ($payType) {
                ...
                case PayServices::YUE_PAY:
                    ...
                    if ($pay['status'] === true){
                        ...
                        $fuyouServices->adjustFuyouByUid($userInfo['uid'],$userInfo['now_money']);
                ...
                case PayServices::ALIAPY_PAY:
                    ...
                    [$url, $param] = strpos($quitUrl, '?') ? explode('?', $quitUrl) : [$quitUrl, ''];
                    if ($param) {
...
//        }
        ...
        switch ($order['pay_type']) {
            ...
            case PayServices::YUE_PAY:
                ...
                if ($pay['status'] === true)
                    ...
                    // $userInfo = $userServices->get($request->uid());
                    // $FuyouServices = app()->make(FuyouServices::class);
                    // $user=$FuyouServices->queryUserInfo($userInfo['phone']);
                    // $fuyoures=$FuyouServices->adjust(,$user['data']['openId'],$userInfo['now_money']*100,"");
    ...
    public function detail(Request $request, StoreOrderEconomizeServices $services, StoreOrderPromotionsServices $storeOrderPromotiosServices, $uni)
    {
        ...
        $orderData['is_apply_refund'] = $orderData['total_num'] > $refund_num;
     ...
     * @throws \think\exception\DbException
     */
    public function comment(Request $request, StoreOrderCartInfoServices $cartInfoServices, StoreProductReplyServices $replyServices, StoreOrderCommentServices $storeOrderCommentServices, UserCollageServices $collageservices)
    {
