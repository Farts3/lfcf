...
class LoginController
{
    ...
    public function user_binding_phone(Request $request)
    {
        ...
        if ($re) {
            ...
            $user=$userServices->getOne(array('phone'=>$phone));
            if(!empty($user)){
                // $thisuser=$userServices->getOne($uid);
                $userdata['now_money']=$user['now_money'];
                $userdata['account']=$user['account'];
                $userServices->eidtNickname($uid,$userdata);
            }
            CacheService::delete('code_' . $phone);
    ...
    public function testcode()
    {
        ...
        return app('json')->success($userMoneyServices->getUserMoneyList(0, $where_time = [], '*')); 
        $FuyouServices = app()->make(FuyouServices::class);
        $phone = '15272213040';
        $beginDate='2023-07-24';
        $endDate='2023-07-24';
        //$rs = $FuyouServices->getFuyouChangeList(21);
         //$rs = $FuyouServices->adjustFuyouByUid(21);
        //$rs = $FuyouServices->queryMemOrderList($phone,$beginDate,$endDate);
       //$rs = $FuyouServices->queryBalance($phone);
       
        /*$userdata=Db::name("user")->where('uid',21)->select();
	    $fuyouservices = app()->make(FuyouServices::class);
	    ...
	    //return $rs;
	     $rs = $FuyouServices->editFuyouUserInfo('18372551416','111111');
