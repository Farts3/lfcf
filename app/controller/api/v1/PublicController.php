...
class PublicController extends BaseController
{
    ...
    {
        [$latitude, $longitude, $product_id, $type, $is_store] = $request->getMore([
            ['latitude', ''],
            ['longitude', ''],
            ['product_id', 0],
		...
		$where = ['type' => 0, 'is_store' => $is_store,'id'=>2];
        $storelist= $services->getStoreList($where, ['*'], $latitude, $longitude, (int)$product_id, [], (int)$type);
	...
	* @return mixed
	...
	public function Synchronize(){
	    $userdata=Db::name("user")/*->where('delete_time',null)*/->select();
	    $fuyouservices = app()->make(FuyouServices::class);
	    foreach ($userdata as $k=>$v){
	        if(isset($v['phone'])&&!empty($v['phone'])) {
	            $rs = $fuyouservices->handleFuyouChange($v);
            }
	    }
	    return true;
	    ...
	    foreach ($userdata as $k=>$v){
	        if(isset($v['phone'])&&!empty($v['phone'])){
                    $FuyouServices = app()->make(FuyouServices::class);
                    $user=$FuyouServices->queryUserInfo($v['phone']);
                    if($user['resultCode']=="000000"){
                        $queryBalance=$FuyouServices->queryBalance($v['phone']);
                        if($queryBalance['resultCode']=="000000"){
                            if(($queryBalance['balance']/100)>$v['now_money']){
                                $FuyouServices->adjust($v['phone'],$user['data']['openId'],$v['now_money']*100,'123456');
                            }else{
                                // adjust($userInfo['phone'],$user['data']['openId'],$userInfo['now_money']*100,'123456');
                                $v['now_money']=$queryBalance['balance']/100;
                                // $updatedata=json_encode($v);
                                // $updatedata=json_decode($v,true);
                                Db::name("user")->where("uid",$v['uid'])->update($v);
                            }
                        }
                        // echo json_encode($queryBalance);die;
                    }else{
                        $res=$FuyouServices->registerUser($v['phone'],"123456",time(),$v['nickname'],01,"");
	...
	public function fuyouOrderPush(Request $request)
    {
        $data = $request->param();
        Log::error($request->header('user-agent').'富友来了'.json_encode($data,JSON_UNESCAPED_UNICODE));
        return response(json_encode(['resultCode'=>'000000','resultMsg'=>'成功']));
        //return json_encode(['resultCode'=>'000000','resultMsg'=>'成功']);
     ...
     * 
     * */ 
    public function getQiSongPrice() {
        $data = Db::name('system_config') -> field('value') -> where('id', 711) -> find();
        $data['value'] = (int)trim($data['value'], '"');
        return app('json')->successful($data);
    }
     ...
     */ 
     public function getStoreCategory() {
        $data = Db::name('category') -> where('is_show', 1) -> select() -> toArray();
        if(count($data) > 0) {
            unset($data[0]); // 删除默认
        }
    ...
    public function getStoreTwoCategory(Request $request) {
        $cate_id = $request -> get('cate_id');
        $data = Db::name('category') -> where(['is_show' => 1, 'pid' => $cate_id]) -> select() -> toArray();
