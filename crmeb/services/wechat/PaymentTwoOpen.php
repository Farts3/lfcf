 ...
 */
class PaymentTwoOpen extends BaseApplication
{
    ...
    public function application()
    {
        ...
        switch ($accessEnd = $this->getAuthAccessEnd($request)) {
            ...
            case self::MINI:
                ...
                $config['app_id'] = $make->get('appId');
                $config['notify_url'] = trim($make->getConfig(DefaultConfig::COMMENT_URL)) . '/api/mp/pay_notify/routine';
                break;
        }
        ...
        if ($this->isV3PAy) {
            ...
            $response = $this->application()->handlePaidNotify(function ($notify, $fail) {
                ...
                if (isset($notify['out_trade_no'])) {
                    $res = Event::until('pay.notify', [$notify]);
                      var_dump('微信支付成功回调接口',$res); return;
                    if ($res) {
                        return $res;
                    } else {
                        return $fail('支付通知失败');
