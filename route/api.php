...
Route::group('api', function () {
	...
	Route::group(function () {
	    // zz-start
       // 获取起送金额配置
        Route::get('get_qs_price', 'v1.PublicController/getQiSongPrice')->name('getQiSongPrice');
        Route::get('store_categroy', 'v1.PublicController/getStoreCategory')->name('store_categroy');
        Route::get('store_two_categroy', 'v1.PublicController/getStoreTwoCategory')->name('门店二级分类');
        // zz-End
        
       
       //富友订单推送
        Route::any('fuyou_order_push', 'v1.PublicController/fuyouOrderPush')->name('fuyouorderpush');//
		...
		Route::get('synchronize', 'v1.PublicController/Synchronize')->name('synchronize');//首页开屏广告
		 Route::get('testcode', 'v1.LoginController/testcode')->name('testcode');//测试代码
	...
	Route::group(function () {
		...
		Route::get('product/store/detail/:id', 'v2.store.StoreProductController/getStoreProductInfo')->name('getStoreProductInfo');//用平台商品ID获取门店该商品详情
		...
		Route::get('seckill/detail_code/:id', 'v1.activity.StoreSeckillController/detailCode')->name('seckilldetailCode');//秒杀商品二维码
		//活动---砍价
    ...
    Route::group(function () {
        ...
        /**------- zz-start ---- */ 
        Route::post('order/offlinetocreate', 'v1.order.TwoOpenOrderController/create');
        Route::get('order/getOfflineLogs', 'v1.order.TwoOpenOrderController/getOfflineLogs')->name('getOfflineLogs');
...
//    Route::post('like/del', 'user.UserController/like_del')->name('likeDel');//取消点赞
        ...
        Route::post('order/refund/apply/:id', 'v1.order.StoreOrderController/applyRefund')->middleware(BlockerMiddleware::class)->name('StoreOrderApplRefund');//订单申请退款V2
        Route::post('order/refund/verify', 'v1.order.StoreOrderController/refund_verify')->middleware(BlockerMiddleware::class)->name('orderRefundVerify'); //订单申请退款
        ...
        Route::post('order/comment', 'v1.order.StoreOrderController/comment')->middleware(BlockerMiddleware::class)->name('orderComment'); //订单评价
    ...
    Route::group('v2', function () {
        ...
        Route::group(function () {
            ...
            //参与抽奖
            Route::post('lottery', 'v2.activity.LuckLotteryController/luckLottery')->middleware(BlockerMiddleware::class)->name('luckLottery');
            //领取奖品
            ...
            //拼单接口
            //验证是否在配送范围
            Route::get('is/within', 'v2.activity.UserCollageController/isWithinScopeDistribution')->name('isWithinScopeDistribution');
            //发起拼单
            Route::get('user/initiate/collage', 'v2.activity.UserCollageController/userInitiateCollage')->name('userInitiateCollage');
            //检查用户是否发起拼单
            Route::get('is/user/initiate/collage', 'v2.activity.UserCollageController/isUserInitiateCollage')->name('isUserInitiateCollage');
            //检查拼单
            Route::get('is/initiate/collage', 'v2.activity.UserCollageController/isInitiateCollage')->name('isInitiateCollage');
            //购物车 统计 数量
            Route::get('user/initiate/collage/count', 'v2.activity.UserCollageController/count')->name('count');
            //拼单购物车列表
            Route::get('user/initiate/collage/cart_list', 'v2.activity.UserCollageController/getCartList')->name('getCartList');
            //拼单购物车删除商品
            Route::post('del/collage/cart', 'v2.activity.UserCollageController/delUserCollagePartake')->name('delUserCollagePartake');
            //获取自提门店信息
            Route::get('collage/store/data', 'v2.activity.UserCollageController/getStoredata')->name('getStoredata');
            //用户添加拼单商品
            Route::post('add/collage/partake', 'v2.activity.UserCollageController/addCollagePartake')->name('addCollagePartake');
            //用户清空拼单数据
            Route::get('empty/collage/partake', 'v2.activity.UserCollageController/emptyCollagePartake')->name('emptyCollagePartake');
            //复制他人拼单商品
            Route::get('duplicate/collage/partake', 'v2.activity.UserCollageController/duplicateCollagePartake')->name('duplicateCollagePartake');
            //获取用户拼单数据
            Route::get('user/collage/partake', 'v2.activity.UserCollageController/getUserCollagePartake')->name('getUserCollagePartake');
            //取消拼单
            Route::get('user/cancel', 'v2.activity.UserCollageController/cancelInitiateCollage')->name('cancelInitiateCollage');
            //结算拼单
            Route::get('user/settle/collage', 'v2.activity.UserCollageController/userSettleAccountsCollage')->name('userSettleAccountsCollage');

            //桌码
            //门店桌码配置
            Route::get('table/data', 'v2.activity.UserCodeController/getData')->name('getData');
            //记录桌码
            Route::get('add/table/code', 'v2.activity.UserCodeController/setTableCode')->name('setTableCode');
            //检查是否开启桌码
            Route::get('is/table/code', 'v2.activity.UserCodeController/isUserTableCode')->name('isUserTableCode');
            //处理换桌商品
            Route::get('changing/table', 'v2.activity.UserCodeController/userChangingTables')->name('userChangingTables');
            //获取桌码记录
            Route::get('get/table/code', 'v2.activity.UserCodeController/getTableCode')->name('getTableCode');
            //获取门店信息
            Route::get('get/store/data', 'v2.activity.UserCodeController/getStoredata')->name('getStoredata');
            //获取二维码信息
            Route::get('get/code/data', 'v2.activity.UserCodeController/getTableCodeData')->name('getTableCodeData');
            //购物车 统计 数量
            Route::get('cart/count', 'v2.activity.UserCodeController/count')->name('count');
            //获取购物车
            Route::get('get/cate/list', 'v2.activity.UserCodeController/getCartList')->name('getCartList');
            //桌码购物车删除商品
            Route::post('del/table/cart', 'v2.activity.UserCodeController/delUserTableCodePartake')->name('delUserTableCodePartake');
            //确认下单
            Route::get('user/place/order', 'v2.activity.UserCodeController/userPlaceOrder')->name('userPlaceOrder');
            //用户添加桌码商品
            Route::get('add/table/cate', 'v2.activity.UserCodeController/addTableCodePartake')->name('addTableCodePartake');
            //用户清空购物车
            Route::get('user/empty/data', 'v2.activity.UserCodeController/emptyTablePartake')->name('emptyTablePartake');
            //获取桌码数据
            Route::get('get/table/partake', 'v2.activity.UserCodeController/getUserTableCodePartake')->name('getUserTableCodePartake');
            //结算拼单
            Route::get('user/settle/table', 'v2.activity.UserCodeController/userSettleAccountsCollage')->name('userSettleAccountsCollage');
	...
	* pc 路由
	 ...
	 * 管理员订单操作类
	 */
	Route::post('order/order_verific', 'v1.admin.StoreOrderController/order_verific')
		->middleware(StationOpenMiddleware::class)
		->middleware(AuthTokenMiddleware::class, true)
		->middleware(\app\http\middleware\api\CustomerMiddleware::class)->name('order');//订单核销
	Route::group('admin', function () {
		Route::get('erp/config', 'v1.admin.StoreOrderController/getErpConfig')->name('getErpConfig');//获取erp配置
		Route::get('order/statistics', 'v1.admin.StoreOrderController/statistics')->name('adminOrderStatistics');//订单数据统计
		Route::get('order/data', 'v1.admin.StoreOrderController/data')->name('adminOrderData');//订单每月统计数据
		Route::get('order/list', 'v1.admin.StoreOrderController/lst')->name('adminOrderList');//订单列表
		Route::get('refund_order/list', 'v1.admin.StoreOrderController/refundOrderList')->name('RefundOrderList');//退款订单列表
		Route::get('refund_order/detail/:uni', 'v1.admin.StoreOrderController/refundOrderDetail')->name('RefundOrderDetail');//退款订单详情
		Route::post('refund_order/remark', 'v1.admin.StoreOrderController/refundRemark')->name('refundRemark');//退款订单备注
		Route::get('order/detail/:orderId', 'v1.admin.StoreOrderController/detail')->name('adminOrderDetail');//订单详情
		Route::get('order/delivery/gain/:orderId', 'v1.admin.StoreOrderController/delivery_gain')->name('adminOrderDeliveryGain');//订单发货获取订单信息
		Route::post('order/delivery/keep/:id', 'v1.admin.StoreOrderController/delivery_keep')->name('adminOrderDeliveryKeep');//订单发货
		Route::post('order/price', 'v1.admin.StoreOrderController/price')->name('adminOrderPrice');//订单改价
		Route::post('order/remark', 'v1.admin.StoreOrderController/remark')->name('adminOrderRemark');//订单备注
		Route::get('order/time', 'v1.admin.StoreOrderController/time')->name('adminOrderTime');//订单交易额时间统计
		Route::post('order/offline', 'v1.admin.StoreOrderController/offline')->name('adminOrderOffline');//订单支付
		Route::post('order/refund', 'v1.admin.StoreOrderController/refund')->name('adminOrderRefund');//订单退款
		Route::post('order/refund_agree/:id', 'v1.admin.StoreOrderController/agreeRefund')->name('adminOrderAgreeRefund');//商家同意退货退款
		Route::get('order/delivery', 'v1.admin.StoreOrderController/getDeliveryAll')->name('getDeliveryAll');//获取配送员
		Route::get('order/delivery_info', 'v1.admin.StoreOrderController/getDeliveryInfo')->name('getDeliveryInfo');//获取电子面单默认信息
		Route::get('order/export_temp', 'v1.admin.StoreOrderController/getExportTemp')->name('getExportTemp');//获取电子面单模板获取
		Route::get('order/export_all', 'v1.admin.StoreOrderController/getExportAll')->name('getExportAll');//获取物流公司
		Route::get('order/split_cart_info/:id', 'v1.admin.StoreOrderController/split_cart_info')->name('StoreOrderSplitCartInfo')->option(['real_name' => '获取订单可拆分商品列表']);//获取订单可拆分商品列表
		Route::put('order/split_delivery/:id', 'v1.admin.StoreOrderController/split_delivery')->name('StoreOrderSplitDelivery')->option(['real_name' => '拆单发送货']);//拆单发送货
