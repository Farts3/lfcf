...
//        Route::get('r', 'Test/rule')->option(['real_name' => '查看路由地址']);
    ...
    Route::group('export', function () {
        ...
        Route::get('userRecharge', 'v1.other.export.ExportExcel/userRecharge')->option(['real_name' => '用户充值导出']);
         ...
         //admin 后台 余额变动导出
        Route::get('userMoneyList', 'v1.other.export.ExportExcel/userMoneyList')->option(['real_name' => '余额变动导出']);
        //admin 后台 用户导出
        Route::get('exportUser', 'v1.other.export.ExportExcel/exportUser')->option(['real_name' => '导出用户']);
        //分销员推广列表
    ...
    Route::group('finance', function () {
        ...
        //余额变动记录列表
        Route::get('user_money_list', 'v1.user.User/userMoneyList')->option(['real_name' => '余额变动记录列表']);
         //余额变动记录列表
        Route::get('user_money_item', 'v1.user.User/userMoneyItem')->option(['real_name' => '余额变动小项']);
        //删除充值记录
    ...
    Route::group('store', function () {
        ...
        //zz 1-22 start;
        Route::get('get_device', 'v1.store.StoreCategory/getDevice')->option(['real_name' => '设备列表']);
        Route::delete('del_device/:id', 'v1.store.StoreCategory/delDevice')->option(['real_name' => '删除设备']);
        Route::post('save_device/:id', 'v1.store.StoreCategory/saveDevice')->option(['real_name' => '保存修改设备信息']);
        // zz 1-22 end 
    ...
    Route::group('user', function () {
        ...
        Route::post('import_user_money', 'v1.user.user/import')->option(['real_name' => '批量导入用户money']);
		//用户批量操作
...
})->prefix('admin.')->middleware(InstallMiddleware::class)->middleware(\app\http\middleware\AllowOriginMiddleware::class);
