      ...
      <div class="conter">
        <div class="bnt acea-row row-middle">
          ...
          <Button type="primary" :disabled="checkPicList.length === 0" @click="checkPics" class="mr10"
            v-if="isShow !== 0" style="width: 100px;">{{uploadName.file_type==1?'使用选中图片':'使用选中视频'}}</Button>
          <Upload :show-upload-list="false" :action="fileUrl" class="mr10 mb10" :before-upload="beforeUpload"
            ...
            style="margin-top: 1px; display: inline-block" v-if="uploadName.file_type==1">
          ...
          <Button type="error" class="mr10" :disabled="checkPicList.length === 0"
            @click.stop="editPicList('图片')">{{uploadName.file_type==1?'删除图片':'删除视频'}}</Button>
          <i-select :value="pids" :placeholder="uploadName.file_type==1?'图片移动至':'视频移动至'" style="width: 250px" class="treeSel">
...
<style scoped lang="stylus">
  ...
  .treeBox {
    ...
    /deep/ .ivu-tree-title-selected span{
