    ...
    <Card :bordered="false" dis-hover class="mt15">
      ...
      >
        <Row>
          ...
          <Col v-if="currentTab == 4" class="mr">
            <FormItem label="会员类型：">
              <Select
            ...
            <Row>
              <Col class="mr">
                ...
                >
                  ...
                  />
                     <Button type="primary" class="ml20 search"  @click="orderSearch">搜索</Button>
                </FormItem>
                <FormItem
                  label="用户搜索："
                  label-for="nickname"
      ...
      <Row type="flex">
        ...
        <Col>
          <Tooltip content="本页至少选中一项" :disabled="selectArr.length ? true : false">
            <Button class="ml15" type="primary" :disabled="!selectArr.length ? true : false" @click="printOreder">打印配货单</Button>
          </Tooltip>
        </Col>
        <!-- <Col>
					<router-link :to="{path:Setting.routePre+'/order/create',query:{isShow:1}}" target="_blank">
  ...
  methods: {
    ...
    getRecharge() {
      ...
      orderRecharge(this.rechargeData)
        .then((res) => {
          ...
          this.total = data.count
          this.loading = false
          this.tablists.recharg = data.count
        })
        .catch((err) => {
          this.loading = false
          this.$Message.error(err.msg)
    ...
    async exports() {
      ...
      if (
        ...
        orderData.ids = this.selectArr.join(",");
      } else if (this.currentTab == 3) {
