...
export default {
  ...
  methods: {
    ...
    cancel() {
      this.$router.push({ path: `${Setting.routePre}/product/index` });
    },
    ...
    getInfo() {
      ...
      productInfoApi(that.productId || this.$route.query.copy)
        .then((res) => {
          ...
          let deliveryType = data.delivery_type;
          if (Array.isArray(deliveryType)) {
            let index = deliveryType.findIndex((item) => item == 1);
            if (index != -1) {
              deliveryType.splice(index, 1);
