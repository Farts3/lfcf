			...
			<view class='labelNav acea-row row-around row-middle' :style="{top:(sysHeight+40) + 'px'}">
				...
				<view class='nav acea-row row-between-wrapper' :style="{top:(sysHeight+78) + 'px'}">
						...
						<view class='list'>
							<checkbox-group @change="checkboxChange">
								<view class="title acea-row row-between-wrapper"
									v-if="cartList.valid.length && Object.prototype.toString.call(discountInfo.coupon) === '[object Object]'">
									<view style="width:530rpx;"><text
											class="label">优惠券</text>最高可优惠{{discountInfo.deduction.coupon_price}}元</view>
									<view class="font-color" @click="couponTap">点击领用<text
											class="iconfont icon-jinru2"></text></view>
								</view>
								<view v-for="(j,jindex) in cartList.valid" :key="jindex">
									<view class="title acea-row row-between-wrapper"
										v-for="(p,proIndex) in j.promotions" :key="proIndex">
										<view style="width:530rpx;"><text
												class="label">{{p.title}}</text>{{p.desc}}<text
												v-if="p.differ_threshold>0">,还差{{p.differ_threshold}}{{p.threshold_type==1?'元':'件'}}</text>
										</view>
										<view class="font-color" @click="goCollect(p)">
											{{p.is_valid == 1?'去逛逛':'去凑单'}}<text class="iconfont icon-jinru2"></text>
										</view>
									</view>
									<!-- <checkbox-group @change="checkboxChange"> -->
											...
											<view class='picTxt acea-row row-between-wrapper'>
												...
												<view class='text'>
													...
													<view class='infor line1'
														...
														<text
															class="name line1">属性：{{item.productInfo.attrInfo.suk}}</text>
													...
													<view class='money' v-if="item.attrStatus && !item.is_gift">
														￥{{item.sum_price}}</view>
												...
												<view class='carnum acea-row row-center-wrapper'
													...
													<view class='num'>
														{{(item.productInfo.limit_num>0 && item.cart_num>=item.productInfo.limit_num)?item.productInfo.limit_num:item.cart_num}}
													</view>
													<!-- <view class="num">
												<input type="number" v-model="item.cart_num" @click.stop @input="iptCartNum(index)" @blur="blurInput(index)"/>
											</view> -->
													<view class="plus"
														:class="(item.numAdd || (item.productInfo.limit_num>0 && item.cart_num>=item.productInfo.limit_num)) ? 'on' : ''"
														@click.stop='addCart(jindex,index,item)'>+</view>
												</view>
											</view>
											<view class="evaluate"
												v-if="item.attrStatus && !item.is_gift && item.sum_price != item.truePrice">
												预估到手价<text class="num">￥{{item.truePrice}}</text></view>
										</view>
									</block>
									<!-- </checkbox-group> -->
								</view>
					...
					<view v-if="cartList.valid.length == 0 && cartList.invalid.length == 0 && loadend"
						style='height:30rpx;color: #F5F5F5;'></view>
					...
					<view class='footer acea-row row-between-wrapper' :class="isFooter?'':'on'"
						...
						<view class='money acea-row row-middle' v-if="footerswitch==true && discountInfo.deduction">
							<view class="left">
								实付：
								<text
									class="font-color">￥{{selectValue.length?discountInfo.deduction.pay_price:0}}</text>
								<view class="acea-row row-right">
									<view class="detailed" @click="discountTap"
							...
							<form @submit="subOrder">
								<button v-if="selectValue.length" class='placeOrder bg-color'
									formType="submit">{{Object.prototype.toString.call(discountInfo.coupon) === "[object Array]" || discountInfo.coupon.used?'去':'领券'}}结算({{selectValue.length}})</button>
								<button v-else class='placeOrder on'
									formType="submit">{{Object.prototype.toString.call(discountInfo.coupon) === "[object Array]" || discountInfo.coupon.used?'去':'领券'}}结算({{selectValue.length}})</button>
							</form>
						</view>
				...
				<!-- 产品属性显示 -->
				<productWindow :attr="attr" :isShow='1' :iSplus='1' :iScart='1' :storeInfo="storeInfo" :is_vip="is_vip"
					@myevent="onMyEvent" @ChangeAttr="ChangeAttr" @ChangeCartNum="ChangeCartNum" @attrVal="attrVal"
					@iptCartNum="iptCartNum" @goCat="reGoCat" id='product-window'></productWindow>
				<!-- 优惠明细显示 -->
				...
				if (selectValue.length > 0) {
					...
					if (Object.prototype.toString.call(coupon) === '[object Object]' && !coupon.used) {
						setCouponReceive(this.discountInfo.coupon.id).then(res => {
							uni.navigateTo({
								url: '/pages/goods/order_confirm/index?cartId=' + selectValue.join(',') +
									...
									'&delivery_type=' + delivery_type + '&store_id=' + storeId
							});
						}).catch(err => {
							return that.$util.Tips({
								title: err
						...
						if (Object.prototype.toString.call(coupon) === '[object Array]') {
							url = '/pages/goods/order_confirm/index?cartId=' + selectValue.join(',') + '&delivery_type=' +
								delivery_type + '&store_id=' + storeId
						} else {
							url = '/pages/goods/order_confirm/index?cartId=' + selectValue.join(',') + '&couponId=' +
								coupon.used.id + '&couponTitle=' + coupon.coupon_title + '&delivery_type=' +
								delivery_type + '&store_id=' + storeId
						}
						uni.navigateTo({
							url: url
