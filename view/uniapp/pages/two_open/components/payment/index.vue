	...
	export default {
		...
		watch: {
			payMode: {
				handler(newV, oldValue) {
					...
					let yue =newPayList.find(item => {
						return item.value == 'yue'
					})
					console.log(yue,newPayList)
					 ...
					 * 修改为默认余额支付
					 * */ 
					this.active = yue.index; 
					this.paytype = yue.value;
					this.number = yue.number || 0;
				},
				immediate: true,
				deep: true
			}
		...
		methods: {
			...
			goPay: function(number, paytype) {
				...
				if (paytype == 'yue' && parseFloat(this.number) < parseFloat(that.totalPrice)) return that.$util.Tips({
					title: '余额不足！'
				});
				uni.showLoading({
					title: '支付中'
				...
				createOrder({
					...
					actual_price: (this.totalPrice * this.payPost.discount / 100).toFixed(2),
					discount: this.payPost.discount,
					user_real_name: this.payPost.user_real_name,
					store_name:this.payPost.store_name,
					store_id:this.payPost.store_id,
				}).then(res => {
