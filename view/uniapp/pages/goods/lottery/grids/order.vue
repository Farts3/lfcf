<template>
	<view :style="colorStyle">
		<view class='order-submission'>
				...
				<view class='headerCon'>
					<view class="add-title acea-row row-between-wrapper">
						...
						<view class="text" @tap='onDelivery'
							v-if="(isDisplay.length >1 || (isDisplay.length==1 && deliveryType !=1)) && store_func_status">
			...
			<view class="wrapper">
				<orderGoods :cartInfo="cartInfo" :giveData="giveData" :shippingType="shippingType"
			...
			<view class='wrapper' v-if="confirm.length">
				<view class='item acea-row row-between-wrapper' v-for="(item,index) in confirm" :key="index">
					...
					<view v-if="item.label=='text'" class="discount">
						<input type="text" :placeholder="'请填写'+item.title" placeholder-class="placeholder"
							v-model="item.value" />
					</view>
					<!-- number -->
					<view v-if="item.label=='number'" class="discount">
						<input type="number" :placeholder="'请填写'+item.title" placeholder-class="placeholder"
							v-model="item.value" />
					</view>
					<!-- email -->
					...
					<view v-if="item.label=='id'" class="discount">
						<input type="idcard" :placeholder="'请填写'+item.title" placeholder-class="placeholder"
							v-model="item.value" />
					</view>
					<!-- phone -->
					<view v-if="item.label=='phone'" class="discount">
						<input type="tel" :placeholder="'请填写'+item.title" placeholder-class="placeholder"
							v-model="item.value" />
					</view>
					<!-- img -->
        ...
        <view class='settlement bg-color-hui' style='z-index:100' v-else>立即结算</view> -->
          ...
          product_type != 0">
				...
				<!-- <view class='settlement' style='z-index:100' @tap.stop="goPay">提交订单</view> -->
			</view>
		</view>
		...
		<payment ref="payment" :payMode="cartArr" :pay_close="pay_close" :isCall="true" :totalPrice="totalPrice.toString()"
		...
		methods: {
			...
			deliveryFun(e, item) {
				...
				if (e == 2) {
					...
					if (e == 3) {
						this.storeId = item.id;
						if (this.isDisplay.length == 2 && this.isDisplay.includes("2") && this.isDisplay.includes("3")) {
							this.getaddressInfo();
			...
			goPay() {
				// this.pay_close = true;
				this.$refs.payment.goPay(this.cartArr[2].number, 'yue');
			},
			payCheck(type) {
				this.payType = type;
				this.SubOrder();
			...
			getList: function(fromType, index) {
				...
				storeListApi(data).then(res => {
					...
					this.$set(this, 'storeId', this.storeId ? this.storeId : list[0].id);
					list.forEach(item => {
