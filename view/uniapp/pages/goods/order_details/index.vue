	...
	<view :style="colorStyle">
		<view class='order-details'>
			...
			<view v-if="isGoodsReturn==false">
				<view class='nav'>
					<view class='navCon acea-row row-between-wrapper'>
						...
						<view :class="status.type == 1 || status.type == 5 ? 'on':''" v-if="orderInfo.shipping_type!=4 && orderInfo.type != 10">
							{{(orderInfo.shipping_type==1 || orderInfo.shipping_type==3) ? '待发货':'待核销'}}
						</view>
						<view :class="status.type == 2 ? 'on':''"
							v-if="(orderInfo.shipping_type == 1 || orderInfo.shipping_type == 3) && orderInfo.type != 10">待收货</view>
						<view v-if="orderInfo.type != 8" :class="status.type == 3 ? 'on':''">待评价</view>
						<view :class="status.type == 4 ? 'on':''">已完成</view>
					</view>
					<view class='progress acea-row row-between-wrapper'>
						<view v-if="orderInfo.type != 10" class='iconfont'
							:class='(status.type == 0 || status.type == -9  ? "icon-webicon318":"icon-yuandianxiao") + " " + (status.type >= 0 ? "font-num":"")'>
						</view>
						<view class='line' :class='status.type > 0 ? "bg-color":""' v-if="orderInfo.shipping_type!=4 && orderInfo.type != 10">
						</view>
						<view class='iconfont'
							:class='(status.type == 1 || status.type == 5 ? "icon-webicon318":"icon-yuandianxiao") + " " + (status.type >= 1 ? "font-num":"")'
							v-if="orderInfo.shipping_type!=4 && orderInfo.type != 10">
						</view>
						<view class='line' :class='status.type > 1 && status.type != 5 ? "bg-color":""'
							v-if="(orderInfo.shipping_type == 1 || orderInfo.shipping_type == 3) && orderInfo.type != 10">
						</view>
						<view class='iconfont'
							:class='(status.type == 2 ? "icon-webicon318":"icon-yuandianxiao") + " " +(status.type >= 2 ? "font-num":"")'
							v-if="orderInfo.shipping_type == 1 || orderInfo.shipping_type == 3 || (orderInfo.shipping_type == 4 && orderInfo.type == 10)"></view>
						<view class='line' :class='status.type > 2 && status.type != 5 ? "bg-color":""'></view>
						<view v-if="orderInfo.type != 8" class='iconfont'
							:class='(status.type == 3 ? "icon-webicon318":"icon-yuandianxiao") + " " + (status.type >= 3 && status.type != 5  ? "font-num":"")'>
						</view>
						<view v-if="orderInfo.type != 8" class='line' :class='status.type > 3 && status.type != 5 ? "bg-color":""'></view>
						<view class='iconfont'
				...
				<view class="writeOff" v-if="orderInfo.verify_code && orderInfo.paid == 1">
					...
					<view class="rules">
						<view class="item" v-if="orderInfo.shipping_type == 2 && orderInfo.product_type==4"> 
							<view class="rulesTitle acea-row row-middle">
								<text class="iconfont icon-tishi"></text>核销信息
							</view>
							<view class="info">
								核销次数：<text class="font-num">{{orderInfo.write_off}}</text>/{{orderInfo.write_times}}
							</view>
						</view>
						<view class="item" v-if="orderInfo.shipping_type == 2 && orderInfo.product_type==4">
							<view class="rulesTitle acea-row row-middle">
							...
							<view class="info">{{orderInfo.write_day}}</view>
						</view>
						<view class="item" v-if="orderInfo.shipping_type == 2">
							...
							<view class="info">
								每日：{{orderInfo.system_store.day_time}}
							</view>
						</view>
						<view class="item">
							<view class="rulesTitle acea-row row-middle">
				...
				<view class='address' v-if="(orderInfo.shipping_type === 1 || orderInfo.shipping_type === 3) && orderInfo.product_type==0 && orderInfo.type != 10">
					<view class='name'>{{orderInfo.real_name}}<text class='phone'>{{orderInfo.user_phone}}</text></view>
					<view>{{orderInfo.user_address}}</view>
				</view>
				<view class='address' v-if="(orderInfo.shipping_type === 2 || orderInfo.shipping_type === 4) && (orderInfo.product_type==0 || orderInfo.product_type==4)">
					<view class='name acea-row' @tap="makePhone">
						<view class="nameCon">{{orderInfo.system_store.name}}</view>
						<text class='phone'>{{orderInfo.system_store.phone}}</text>
						<text class="iconfont icon-tonghua font-num"></text>
					</view>
					<view>{{orderInfo.system_store.detailed_address}}</view>
				</view>
				<view class='line' v-if="orderInfo.shipping_type === 1 && orderInfo.type != 10">
					<image src='@/static/images/line.jpg'></image>
				</view>
			</view>
			<view class="delivery acea-row row-between-wrapper" v-if="orderInfo.delivery_type == 'city_delivery' || orderInfo.delivery_type == 'send'">
				...
				<navigator v-if="orderInfo.delivery_type == 'city_delivery'" :url="'/pages/goods/delivery_detail/index?orderId='+orderInfo.id" hover-class='none' class="details" >查看详情</navigator>
			</view>
			...
			<orderGoods :evaluate='evaluate' :giveData="giveData" :deliveryType="orderInfo.shipping_type" :statusType="status.type"  
				:sendType="orderInfo.delivery_type" :orderId="order_id" :oid="Number(orderInfo.id)" :cartInfo="cartInfo" :pid="pid" :jump="true" :refund_status="orderInfo.refund_status" :paid="Number(orderInfo.paid)" :productType="orderInfo.product_type" :giveCartInfo="giveCartInfo" :is-group="orderInfo.type == 9" :collage-id="orderInfo.activity_id">
			</orderGoods>
			<!-- #ifdef H5 || APP-PLUS -->
			...
			<!-- #ifdef MP -->
			<!-- 桌码信息 -->
			<view v-if="orderInfo.type == 10" class='wrapper'>
				<view class='item acea-row row-between'>
					<view>桌台号：</view>
					<view class='conter'>{{ tableCode.category.name }} {{ tableCode.table_number }}</view>
				</view>
				<view class='item acea-row row-between'>
					<view>就餐人数：</view>
					<view class='conter'>{{ tableCode.number_diners }}人</view>
				</view>
			</view>
			<!-- #endif -->
			<view class='wrapper'>
			...
			<view class='wrapper'>
				...
				<view class='item acea-row row-between' v-if="orderInfo.pay_postage > 0 && orderInfo.type != 10">
					<view>配送运费：</view>
			...
			<view class='footer acea-row row-right row-middle'
				...
				<view class="more" v-if="(invoice_func || invoiceData || status.class_again==6) && orderInfo.paid && !orderInfo.refund_status && orderInfo.type != 8"
				...
				<template v-if="orderInfo.type != 8">
					...
					<view
						...
						class='bnt cancel' v-else-if="orderInfo.paid == 1 && orderInfo.is_apply_refund && orderInfo.refund_status == 0 && cartInfo.length>1">
						批量退款</view>
				</template>
				<!-- #ifdef MP -->
				<!-- <view
				...
				<view class='bnt bg-color' v-if="orderInfo.type==3 && orderInfo.paid && orderInfo.refund_status==0 && orderInfo.type != 8" @tap='goJoinPink'>查看拼团</view>
...
<style scoped lang="scss">
	...
	.order-details .address .name .nameCon{
		max-width: 410rpx;
	}
		...
		methods: {
			...
			getOrderStatus: function() {
				...
				if (!seckill_id && !bargain_id && !combination_id && !discount_id && !orderInfo.type && (type == 3 ||
						type == 4)) status.class_status = 5; //（待评价、已完成）
			...
			goOrderConfirm: function() {
				...
				orderAgain(that.orderInfo.order_id).then(res => {
					let deliveryType = 1,shippingType = this.orderInfo.shipping_type,storeId = this.orderInfo.store_id,storeName = this.orderInfo.system_store.name
					if(shippingType == 1 && storeId == 0){
						deliveryType = 1
					}else if((shippingType == 2 || shippingType == 4) && storeId>0){
						deliveryType = 2
					}else if(shippingType == 1 && storeId > 0){
						deliveryType = 3
					}
					return uni.navigateTo({
						url: '/pages/goods/order_confirm/index?new=1&cartId=' + res.data.cateId + '&delivery_type=' + deliveryType + '&store_id='+storeId+'&store_name='+storeName
					});
				}).catch(err=>{
					return that.$util.Tips({
						title: err
