		...
		methods: {
			...
			subOrder: function() {
				...
				if (list.length) {
					...
					let deliverySort = list[0].productInfo.delivery_type.sort((x,y)=>x - y);
					delivery_type = deliverySort[0];
					storeId = list[0].productInfo.type == 1?list[0].productInfo.relation_id:0
					uni.navigateTo({
						url: '/pages/goods/order_confirm/index?cartId=' + ids.join(',')+'&delivery_type='+delivery_type + '&store_id=' + storeId
					});
