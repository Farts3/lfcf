		...
		<form @submit="formSubmit">
			<view class='personal-data' :style="colorStyle">
				...
				<view class='list'>
					...
					<view class='item acea-row row-between-wrapper'>
						<view>支付密码</view>
						<view class='input'><input type='number' password name='fuyou_pwd' v-model='fuyou.pwd' maxlength="6"></input>
						</view>
					</view>
					<view class='item acea-row row-between-wrapper'>
						<view>确定密码</view>
						<view class='input'><input type='number' password name='fuyou_pwd' v-model='fuyou.pwd1' maxlength="6"></input>
						</view>
					</view>
	...
	export default {
		...
		methods: {
			...
			formSubmit: function(e) {
				...
				if(this.fuyou.pwd || this.fuyou.pwd1){
					if(that.fuyou.pwd.length != 6 || !/^\d+$/.test(this.fuyou.pwd)) {
						return that.$util.Tips({
							title:'支付密码为6位纯数字'
						})
					}
					
					if(that.fuyou.pwd !== that.fuyou.pwd1) {
						return that.$util.Tips({
							title:'两次密码不一致'
						})
					}
