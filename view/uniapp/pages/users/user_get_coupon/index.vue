			...
			<view class="item acea-row row-between-wrapper" v-for="(item,index) in couponsList" :key="index" :class="{svip: item.receive_type === 4}">
				<view class="left" v-if="item.products.length>1 || item.products.length == 0">
					<view class="name line1" :class='item.is_use ? "moneyGray" : ""'>
						...
						<text class="label" :class="item.is_use ? 'bg-color-huic' : ''" v-else-if="item.type === 2">商品券</text>
						<text class="label" :class="item.is_use ? 'bg-color-huic' : ''" v-else>品牌券</text>
						{{ item.title }}
					</view>
					...
					<view class="text">
						<view class="top">
							...
							<view class="acea-row">
								...
								<view class="label" :class="item.is_use ? 'bg-color-huic' : ''" v-else-if="item.type === 2">商品券</view>
								<view class="label" :class="item.is_use ? 'bg-color-huic' : ''" v-else>品牌券</view>
							</view>
						</view>
