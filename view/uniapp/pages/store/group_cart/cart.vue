      ...
      <view class="store-address">
		...
		<view class="switch acea-row row-between-wrapper">
			<template v-for="(item, index) in deliveryList">
				<view v-if="item.key == switchNum" class="title on" :key="item.key" :class="{'onLeft':switchNum != 1}">{{item.name}}</view>
			</template>
		</view>
      </view>
      <!-- 门店详情 -->
      ...
      <view class="store-address">
        ...
        <view class="time">
          营业时间：{{info.day_time || '-'}}
			...
			<view class="switch acea-row row-between-wrapper">
				<template v-for="(item, index) in deliveryList">
					<view :data-id="info.delivery_type" class="title on" :class="{'onLeft':switchNum != 1}" :key="item.key">{{item.name}}</view>
				</template>
			</view>
      </view>
    </view>
          ...
          <view class="wrapper">
            ...
            <view class="footer acea-row row-between-wrapper" :class="isFooter?'':'on'">
              ...
              <view class="acea-row row-middle" :class="cartNum>0?'':'noCart'">
                ...
                <view class="bnt" @click="subOrder">选好了</view>
              </view>
            </view>
			<!-- <addressWindow ref="addressWindow" :address="address" @changeClose="onAddress" @OnChangeAddress="onAddressChange"></addressWindow> -->
            <!-- 分类购物车下拉列表 -->
...
<script>
  ...
  export default {
   ...
   //    goMap(val) {
   //      uni.navigateTo({
			// url: `/pages/store/map/index?isGroup=${val}&storeFrom=1&type=1&storeId=${this.info.id}`,
   //        success(res) {
   //          console.log('成功啦', res);
   //        },
   //        fail(err) {
