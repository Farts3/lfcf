				...
				<view class="head">
                  ...
                  <view class="avatarName">获取头像</view>
                    ...
                    {{userInfo.service_num >= 100 ? '99+' : userInfo.service_num}}
									...
									<uni-badge class="uni-badge-left-margin" v-if="userInfo.service_num>0"
							...
							<block v-for="(item,index) in orderMenu" :key="index">
								<view class="order-item" @click="intoPage(item.url)">
									<view class="pic">
										...
										<uni-badge class="uni-badge-left-margin" v-if="item.num>0" :text="item.num"
											absolute="rightTop"></uni-badge>
...
<script>
	...
	export default {
		...
		methods: {
			...
			// 扫一扫功能，到线下实体店扫码支付
			handleSaoYiSao() {
				console.log('扫一扫功能，到线下实体店扫码支付');
				uni.scanCode({
					onlyFromCamera: false,
					// scanType: ['qrCode'], // u识别不了小程序二维码
					success(res) {
						let decodeUrl = decodeURIComponent(res.path)
						// let a = 'pages/store_cate/store_cate?scene=id=5'
						let idStr = decodeUrl.substring(34, decodeUrl.length)
						console.log('扫码成功',decodeURIComponent(res.path))
						// 扫码成功跳转到线下支付页
						uni.navigateTo({
							url:'/pages/two_open/scanCodeInfo?' + idStr
						})
					}
				})
			},
			...
			getMyMenus: function() {
				...
				getMenuList().then(res => {
					...
					console.log('myMenut', myMenu)
					myMenu.push({id:-1, name:"线下支付", pic:require('../two_open/static/sacn.png'), type:1,url:'/pages/two_open/sacnLog'})
