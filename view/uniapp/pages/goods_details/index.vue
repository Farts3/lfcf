				...
				<view id="home" class="home acea-row row-center-wrapper" :class="[opacity>0.5?'on':'']"
					...
					<view>客服</view>
							...
							<view class="bnt acea-row skeleton-rect" :class="!storeInfo.cart_button?'fictitious' : ''"
								...
								<form v-if="is_store_buy" @submit="goBuy" class="buy bnts"><button class="buy bnts"
							...
							<view class="bnt acea-row skeleton-rect" :class="!storeInfo.cart_button?'fictitious' : ''"
								...
								<form v-else @submit="goBuy" class="buy bnts"><button class="buy bnts"
										form-type="submit">{{is_store_buy?'门店购买':'立即购买'}}</button>
								</form>
							</view>
						</view>
						...
						<!-- 属性组件 -->
						<productWindow :attr="attr" :isShow="1" :iSplus="1" :cusPreviewImg='1' :storeInfo="storeInfo" :productType="storeInfo.product_type" :productId="storeInfo.id" :isStoreBuy="is_store_buy"
...
<script>
	...
	export default {
		...
		methods: {
			...
			// 门店详情
			getStoreDetail(id){
				getProductStoreDetail(this.id,{store_id:id}).then(res=>{
			...
			showStoreList: function() {
				uni.navigateTo({
					url: '/pages/goods/goods_details_store/index?productId='+this.id
				})
			},
			videoPause() {
				this.$nextTick(() => {
			...
			getGoodsDetails: function() {
				...
				obj(that.id, {
					...
					.then(res => {
						...
						// 	that.$set(that.limitInfo, 'discount_price', discountPrice);
						// }
						that.$set(that, 'is_store_buy', res.data.is_store_buy);
