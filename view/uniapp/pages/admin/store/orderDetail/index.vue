<template>
	<view class="order-details pos-order-details">
		...
		<view class="wrapper">
			<view class="item acea-row row-between">
				<view>商品总价：</view>
				<view class="conter" v-if="statusType == -3">￥{{orderInfo.total_price}}</view>
				<view class="conter" v-else>￥{{(parseFloat(orderInfo.total_price)+parseFloat(orderInfo.vip_true_price)).toFixed(2)}}</view>
			</view>
		...
		<view class="footer acea-row row-right row-middle">
			...
			<view class="bnt cancel" :class="openErp?'on':''" @click="modify('2',1)" v-if="orderInfo.type != 8 && (!orderInfo.refund || !orderInfo.refund.length) && (orderInfo.refund_type == 0 || orderInfo.refund_type == 1 || orderInfo.refund_type == 5) && orderInfo.paid && parseFloat(orderInfo.pay_price) >= 0">
				立即退款
			</view>
