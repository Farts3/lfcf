<template>
	<view :style="colorStyle">
					...
					<view class='attribute skeleton-rect'>
						...
						<view class="storeInfo acea-row row-between row-top" v-if="isMention && storeLen" @tap="showStoreList">
							<view class="acea-row row-top row-between">
								门店：
								<storeLis :num='1' :type="'seckill'" :ids="parseInt(storeInfo.id)" @getStoreList="getStoreList"></storeLis>
							</view>
							<view class="iconfont icon-jiantou"></view>
						</view>
					</view>
		...
		<product-window :attr='attribute' :limitNum='1' :cusPreviewImg='1' :productType="storeInfo.product_type" :productId="storeInfo.id" @myevent="onMyEvent" @ChangeAttr="ChangeAttr" :type="'seckill'"
			@ChangeCartNum="ChangeCartNum" @attrVal="attrVal" @iptCartNum="iptCartNum" @getImg="showImg" @deliveryFun="deliveryFun" @onAddressId="onAddressId" @onstoreId="onstoreId"></product-window>
		<!-- 分享按钮 -->
			...
			goCat: function() {
				if(this.isLogin){
					...
					postCartAdd({
						...
						this.isOpen = false
						uni.navigateTo({
						  url: '/pages/goods/order_confirm/index?new=1&cartId=' + res.data.cartId+'&delivery_type='+that.delivery_type+'&addressId='+that.addressId+'&store_id='+that.store_id+'&store_name='+that.store_name+'&product_id=' + that.storeInfo.id
						});
					}).catch(err => {
						return this.$util.Tips({
							title: err
