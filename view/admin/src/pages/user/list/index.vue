<template>
  ...
  <div>
                  ...
                  <Row>
                    <Col>
                      <FormItem label="用户搜索：" label-for="nickname">
                        <Input v-model="userFrom.nickname" placeholder="请输入" element-id="nickname" clearable class="input-add">
                ...
                <Row>
                  <Col>
                    <FormItem label="用户分组：" label-for="group_id">
                      <Select v-model="userFrom.group_id" placeholder="请选择" element-id="group_id" clearable class="input-add">
                ...
                <Row>
                  <Col>
                    <FormItem label="会员等级：" label-for="level">
                      <Select v-model="userFrom.level" placeholder="请选择" element-id="level" clearable class="input-add">
                        <!--                        <Option value="">全部</Option>-->
                ...
                <Row>
                  ...
                  <Col>
                    <FormItem label="访问情况：" label-for="user_time_type">
                      <Select v-model="userFrom.user_time_type" placeholder="请选择" element-id="user_time_type" clearable class="input-add">
                        <!--                        <Option value="all">全部</Option>-->
                  ...
                  <Col>
                    <FormItem label="下单次数：" label-for="pay_count">
                      <Select v-model="userFrom.pay_count" placeholder="请选择" element-id="pay_count" clearable class="input-add">
                        <!--                        <Option value="">全部</Option>-->
                ...
                <Row>
                  <Col>
                    <FormItem label="国家：" label-for="country">
                      <Select v-model="userFrom.country" placeholder="请选择" element-id="country" clearable @on-change="changeCountry" class="input-add">
                        <!--                        <Option value="">全部</Option>-->
                  ...
                  <Col v-if="userFrom.country === 'domestic'">
                    <FormItem label="省份：">
                      <Cascader :data="addresData" :value="address" v-model="address" @on-change="handleChange" class="input-add"></Cascader>
                    </FormItem>
                  </Col>
                </Row>
              </Col>
      ...
      <Row type="flex" justify="space-between">
        ...
        <Col>
          ...
          <Tooltip content="本页至少选中一项" :disabled="!!checkUidList.length && isAll == 0">
            <Button v-auth="['admin-user-coupon']" type="primary" class="mr20" :disabled="!checkUidList.length && isAll == 0" @click="onSend">发送优惠券</Button>
          </Tooltip>
          <Tooltip content="本页至少选中一项" :disabled="!!checkUidList.length && isAll == 0">
            <Button v-auth="['admin-wechat-news']" class="greens mr20" size="default" :disabled="!checkUidList.length && isAll == 0" @click="onSendPic" v-if="userFrom.user_type === 'wechat'">
              <Icon type="md-list"></Icon>
              发送图文消息
            </Button>
          </Tooltip>
          <Tooltip content="本页至少选中一项" :disabled="!!checkUidList.length && isAll == 0">
            <Button v-auth="['admin-user-set_label']" class="mr20" :disabled="!checkUidList.length && isAll == 0" @click="setBatch">批量设置</Button>
          </Tooltip>
          <Button type="primary" class="mr20" @click="exportUserList">导出会员信息</Button>
        </Col>
        <Col>
          <Upload style="display: inline-block;" action="" :show-upload-list="false" accept=".xls,.xlsx" :before-upload="bulkRecharge">
            <Button type="primary" class="mr20">批量充值</Button>
          </Upload>
          <Button type="primary" @click="templateDownload">模板下载</Button>
        </Col>
      </Row>
      <!-- 用户列表表格 -->
      <vxe-table
    ...
    <!-- 用户标签 -->
    <Modal v-model="labelListShow" scrollable title="请选择用户标签" :closable="false" width="320" :footer-hide="true" :mask-closable="false">
    ...
    <!-- 发送优惠券-->
    <send-from ref="sends" :is-all="isAll" :where="userFrom" :userIds="checkUidList.join(',')"></send-from>
    <!-- 会员详情-->
...
export default {
  ...
  methods: {
    templateDownload() {
      location.href = 'http://60.205.4.111:8888/down/WZFEkTSrD0fN.xls'
    },
    async bulkRecharge(file) {
      const formData = new FormData()
      formData.append('file', file)
      const { data } = await uploadToBulkRecharge(formData).catch(res => {
    ...
    async exportUserList() {
      ...
      for (let i = 0; i < excelData.page + 1; i++) {
        let lebData = (await getExportUserList({
          ...excelData,
          uid: this.checkUidList
        })).data
